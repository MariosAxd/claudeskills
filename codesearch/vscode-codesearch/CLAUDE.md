...
```
cd codesearch/vscode-codesearch
...
**Development (F5):**
Open `codesearch/vscode-codesearch/` as a VS Code workspace, press **F5**. A new Extension Development Host window opens with the extension loaded.

**Install from folder (no packaging):**
F1 → `Developer: Install Extension from Location…` → select `codesearch/vscode-codesearch/`.

**Package and install:**
```
npm install -g @vscode/vsce   # once
codesearch\setup-vscode-ext.cmd
```
...
**Path resolution for WSL roots.** `resolveFilePath()` in `client.ts` converts `/mnt/q/...` WSL paths to `Q:/...` Windows paths before calling `vscode.Uri.file()`, because the extension host runs on Windows even when Typesense is running in WSL.
